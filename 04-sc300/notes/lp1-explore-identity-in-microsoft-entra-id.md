...
- Check your knowledge
  - 1 What are the three tenants of Zero Trust?
    - Verify explicitly - Least privilege - Assume breach **Correct**
    - Verify explicitly - Least privilege - Conditional access
    - Multifactor authentication - Conditional access - Privilege identity management


  - 2 What is decentralized identity?
    - Storing all your user and device identities in Active Directory on-premises and then using Microsoft Entra Connect to apply them in the cloud.
    - An identity provider that provides verified identities provided in a blockchain store. **Correct**
    - Microsoft Entra ID

  - 3.What is claims-based authentication?
    - When a user signs in, Microsoft Entra ID creates and sends an Access token that contains a set of claims about the user. **Correct**
    - When a user doesn't need to sign in, but sends a claim directly to a cloud resource to access it. The claim is in the form of a JSON Web Token, and provides the user sign-in information.
    - When a user signs in, Microsoft Entra ID writes every access privilege the user has into a token and shares it with each application. 
